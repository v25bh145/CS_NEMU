...
make_helper(concat(decode_si_, SUFFIX)) {
	...
	op_src->simm = ???
	 */
	// panic("please impleme me %d", (DATA_TYPE_S)instr_fetch(eip, DATA_BYTE));
...
make_helper(concat(decode_i_condition_, SUFFIX)) {
	...
	if(op_src->abstract_instr_enum == 0x0f)
		op_src->abstract_instr_enum = instr_fetch(cpu.eip + 1, 1);
...
make_helper(concat(decode_rmw2r_,SUFFIX)) {
	...
	op_src->size = 2;
	// panic("len: %x", eip - 1);
	int len_addr = read_ModR_M(eip - 1, op_src, op_dest);
...
//myself
void concat(testfor_flags_s_, SUFFIX) (DATA_TYPE_S ret, uint32_t type) {
	//type for:
	...
	if((type & 0x800) != 0) {
    	...
    	cpu.psw->OF = (tmp1 == tmp2 && tmp1 != ret >> ((DATA_BYTE << 3) - 1));
	}
    ret ^= ret >> 4;
    ret ^= ret >> 2;
    ret ^= ret >> 1;
